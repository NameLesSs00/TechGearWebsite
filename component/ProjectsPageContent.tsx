            ...
            projects.map((project) => (
              <article key={project.id} className="grid overflow-hidden rounded-[1.5rem] border border-[#22D3EE] bg-[#1b2635] shadow-[0_0_20px_rgba(34,211,238,0.08)] lg:min-h-[540px] lg:grid-cols-[1.05fr_.95fr]">
                <div className="flex flex-col items-start justify-center p-8 sm:p-12 lg:p-9 xl:p-14">
                  <span className="inline-flex min-w-[180px] items-center justify-center rounded-full bg-[#6ba2ad] px-7 py-3 text-sm font-medium text-white sm:text-base">
                    {filters.find((filter) => filter.value === project.category)?.[language] ?? ""}
                  </span>
                  ...
                  <Link href={`/${locale}/work/${project.slug}`} className="mt-8 inline-flex min-w-[235px] items-center justify-center gap-2 rounded-full bg-[#22D3EE] px-7 py-3.5 text-sm font-bold text-[#00121F] transition-transform hover:scale-105">
                    {t("work_view_project")} <ArrowRight size={18} className={direction === "rtl" ? "rotate-180" : ""} />
                  </Link>
                </div>
